...
def train(fold_idx=1):
    ...
    for epoch in range(config['num_epoch']):
        ...
        torch.save(net.state_dict(), os.path.join(config['save_folder'], 'DeepLab_v2_LargeFOV.newest.pkl'))
        
        if DSC_MAX <= DSC:
            DSC_MAX = DSC
            torch.save(net.state_dict(), os.path.join(config['save_folder'], 'DeepLab_v2_LargeFOV.pkl'))
